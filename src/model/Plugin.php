...
class Plugin extends SqlElement {
    ...
    private static function getTriggeredEventList() {
      ...
      foreach ($listEvt as $evt) {
        ...
        self::$_triggeredEventList[$evt->event][$evt->className][$evt->id]=$evt->script;
      }
    ...
    public static function checkCustomDefinition($paramClass=null) {
      global $lastFixedField;
      $list=array();
      if ($paramClass) {
        $list[]=$paramClass;
      } else {
      ...
      foreach ($list as $class) {
        $obj=new $class();
        $obj->id=1; // Avoid attributes set only for creation
        if (method_exists($obj,'setAttributes')) $obj->setAttributes();
        $last=false;
        foreach ($obj as $fld=>$val) {
          if ($fld=='_Note' or $fld=='_Link' or $fld=='_Attachment' or $fld=='_sec_Link' or $fld=='_OtherClient' or $fld=='_nbColMax') {
            $last=true;
...
//             continue; // specific field
      ...
      if (count($errorClass)==0) {
        ...
        echo '<br/><div class="messageWARNING" style="text-align:left;height:'.(($paramClass)?'75':'43').'px">';
        ...
        if (!$allFixed) {
          ...
          if (substr($msg,0,1)=='[') $msg="Please access each screen on customization plugin to fix the issue (Plugin V6.1 or over is required)";
          echo $msg;
          traceLog($msg);
        } else {
          ...
          echo $msg;
          if ($paramClass) {
            echo "<div style='width:100%;height;20px;text-align:center;font-weight:bold;color:red;padding-top:10px;'>".i18n('fixCustoRetry')."</div>";
          }
          $plgCusto=SqlElement::getSingleSqlElementFromCriteria('Plugin', array('name'=>'screenCustomization','idle'=>'0'));
          if ($plgCusto and $plgCusto->id and self::afterVersion($plgCusto->pluginVersion,'6.1')) {
            echo "<input type='hidden' id='pluginCustomFixDefinition' value='$class".'::'."$lastFixedField' />";
          } else {
