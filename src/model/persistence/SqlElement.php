...
// END - ADD BY TABARY - NOTIFICATION SYSTEM      
      ...
      if (SqlList::getIdFromTranslatableName ( 'Indicatorable', $classIndicatorable ) and ! $doNotTriggerAlerts) {
        ...
        foreach ( $lstInd as $ind ) {
          ...
          if (SqlElement::is_a($this, 'PlanningElement')) {
            ...
            if(property_exists($objType,'id'.get_class($objType).'Type'))$fldVal=$objType->$fldType;
          } else {
            $objType=$this;
            if(property_exists($objType,'id'.get_class($objType).'Type'))$fldVal=$this->$fldType;
          }
  ...
  public function getSqlElementsFromCriteria($critArray, $initializeIfEmpty = false, $clauseWhere = null, $clauseOrderBy = null, $getIdInKey = false, $withoutDependentObjects = false, $maxElements = null) {
    ...
    if ($clauseWhere) $clauseWhere=str_replace(array(' in ()',' IN ()'),array(' in (0)',' IN (0)'),$clauseWhere); // Should not be mandatory, but will avoid several coding issue
    $whereClause = '';
    $objects = array();
    ...
    if (property_exists ( $this, $idType )) {
      ...
      if (property_exists ( $objType, 'mandatoryContact' ) and $objType->mandatoryContact and property_exists ( $this, 'idContact' ) and property_exists ( $this, 'done' )) {
        if ($this->done and ! $this->idContact) {
          $result = str_replace ( '<br/>' . i18n ( 'messageMandatory', array($this->getColCaption ( 'idContact' )) ), '', $result );
      ...
      if (property_exists ( $objType, 'mandatoryRecipient' ) and $objType->mandatoryRecipient and property_exists ( $this, 'idRecipient' ) and property_exists ( $this, 'done' )) {
        if ($this->done and ! $this->idRecipient) {
          $result = str_replace ( '<br/>' . i18n ( 'messageMandatory', array($this->getColCaption ( 'idRecipient' )) ), '', $result );
    ...
    if (((property_exists ( $type, 'lockHandled' ) and $type->lockHandled) or $force)) {
      if (property_exists ( $this, 'handled' )) {
        if ($status->setHandledStatus) {
          ...
          $this->handled = 0;
          if (property_exists ( $this, 'handledDate' ) and $this->handledDate)$this->handledDate = null;
          if (property_exists ( $this, 'handledDateTime' ) and $this->handledDateTime)$this->handledDateTime=null;
        }
    ...
    if (((property_exists ( $type, 'lockDone' ) and $type->lockDone) or $force)) {
      if (property_exists ( $this, 'done' )) {
        if ($status->setDoneStatus) {
          ...
          $this->done = 0;
          if (property_exists ( $this, 'doneDate' ) and  $this->doneDate) $this->doneDate = null;
          if (property_exists ( $this, 'doneDateTime' ) and $this->doneDateTime)$this->doneDateTime=null;
        }
    ...
    if (((property_exists ( $type, 'lockIdle' ) and $type->lockIdle) or $force) and property_exists ( $this, 'idle' )) {
      if (! self::isSaveConfirmed ()) {
        ...
        if ($status->setIdleStatus) {
          ...
          $this->idle = 0;
          if (property_exists ( $this, 'idleDate' ) and  $this->idleDate)$this->idleDate=null;
          if (property_exists ( $this, 'idleDateTime' ) and  $this->idleDateTime) $this->idleDateTime=null;
          if (property_exists ( $this, 'realEndDate' ) and  $this->realEndDate) $this->realEndDate=null;
        }
      }
    }
