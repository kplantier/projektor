...
class ImputationValidation{
	...
	static function drawUserWorkList($idUser, $idTeam, $idOrga, $startDay, $endDay,$idProject){
	  ...
	  if(!isset($noResource)){
  	  if($idUser == '' and (trim($idTeam)!= '' or trim($idOrga) !='' )){
  	    unset($userVisbileResourceList);
  	    foreach (getUserVisibleResourcesList(true,false,false,false,false,false,false,false,$idProject) as $id=>$name){
  	      $res = new Resource($id, true);
  	      if(trim($idTeam)!= '' and trim($idOrga)!='' and $res->idTeam == $idTeam and $res->idOrganization == $idOrga){
  	        $userVisbileResourceList[$id]=$name;
  	      }else if(trim($idTeam)!= '' and $res->idTeam == $idTeam and trim($idOrga)==''){
  	        $userVisbileResourceList[$id]=$name;
  	      }else if(trim($idOrga)!='' and $res->idOrganization == $idOrga and trim($idTeam)==''){
  	        $userVisbileResourceList[$id]=$name;
  	      }
	  	...
	  	foreach ($periodValueListSorted as $week){
	  	  ...
	  	  $noData = false;
  			...
  			while ($firstWeekDay<=$lastWeekDay){
  			  ...
  			  $weekDayArray[$week->idResource][$week->periodValue][$firstWeekDay]=(isOpenDay($firstWeekDay, $idCalendar))?round($res->getCapacityPeriod($firstWeekDay),2):0;
