...
//scriptLog('Cron::checkImport()');
  ...
  public static function checkEmails() {	
    ...
    $lstIMb = $inputMb->getSqlElementsFromCriteria(null,null,$where);
    ...
    foreach ($lstIMb as $mb){ 
      ...
      try {
        ...
        if($mb->failedRead >= 5) {
          ...
          $content = mb_substr(imap_last_error().(($mb->idle)?' - mailbox closed':''),0,200).' ,next try in 1 hour.';
      ...
      foreach ($mailsIds as $mailId){
        ...
        if($result == ""){
          ...
          if(trim($bodyHtml)) $ticket->description = $bodyHtml;
