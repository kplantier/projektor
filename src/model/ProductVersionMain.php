...
// ============================================================================**********
  ...
  public function save() {
    ...
    if ($this->idProductVersionType and ! $this->idVersionType) {
      $this->idVersionType=$this->idProductVersionType;
    }
    $old=$this->getOld();
    $this->scope='Product';
    $paramNameAutoformat=Parameter::getGlobalParameter('versionNameAutoformat');
    ...
    if ($this->idStatus!=$old->idStatus) {
      if ( $this->isStarted!=$status->setHandledStatus ) {
        ...
        if ($status->setHandledStatus and ! $this->realStartDate){
          ...
          //if (!$this->initialStartDate) $this->initialStartDate=date ( "Y-m-d" );
          //if (!$this->plannedStartDate) $this->plannedStartDate=date ( "Y-m-d" );
        }else{
      ...
      if ( $this->isDelivered!=$status->setDoneStatus) {
        ...
        if ($status->setDoneStatus and ! $this->realDeliveryDate){
          ...
          //if (!$this->initialDeliveryDate) $this->initialDeliveryDate=date ( "Y-m-d" );
          //if (!$this->plannedDeliveryDate) $this->plannedDeliveryDate=date ( "Y-m-d" );
        }else{
      ...
      if ( $this->isEis!=$status->setIntoserviceStatus) {
        ...
        if ($status->setIntoserviceStatus and ! $this->realEisDate){
          ...
          //if (!$this->initialEisDate) $this->initialEisDate=date ( "Y-m-d" );
          //if (!$this->plannedEisDate) $this->plannedEisDate=date ( "Y-m-d" );
        }else{
      ...
      if ($this->idle!=$status->setIdleStatus) {
        ...
        if ($status->setIdleStatus and ! $this->realEndDate){
          ...
          //if (!$this->initialEndDate) $this->initialEndDate = date ( "Y-m-d" );
          //if (!$this->plannedEndDate) $this->plannedEndDate = date ( "Y-m-d" );
        }else{
    ...
    if (! strpos($result,'id="lastOperationStatus" value="OK"')) {
  	...
  	$compList=array();
  	if ($old->id and $old->idProduct!=$this->idProduct) {
