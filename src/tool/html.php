...
function htmlDrawOptionForReference($col, $selection, $obj=null, $required=false, $critFld=null, $critVal=null, $limitToActiveProjects=true, $limitToActiveOrganizations=true,$showIdle=false,$onProjectSelector=false) { 
...
function htmlEncodeUrl($link) {
  $link=htmlEncode(urldecode($link));
  if (substr($link,0,4)!='http' and substr($link,0,7)!='mailto:' and substr($link,0,4)!='tel:') {
    $link='http://'.$link;
  }
  return $link;
}
...
// BEGIN - ADD BY TABARY - TAKE DISPLAY ATTRIBUTE    
    $display = (strpos($attributes,'hidden')===false?($nobr?' display:inline-block':' display:inline-block'):' display:none');
