...
function getMonthName($month,$maxLength=0) {
  ...
  if ($maxLength) {
    if ($maxLength=='auto') {
      $dispMonth=mb_substr($dispMonth,0,4);
      if (strpos('aàeéèêiîïoôuù',substr($dispMonth,-1))!==false) {
        $dispMonth=mb_substr($dispMonth,0,3);
      }
    } else {
      $dispMonth=mb_substr($dispMonth,0,$maxLength);
    }
  }
  return $dispMonth;
}
...
function activityStreamDisplayHist ($hist,$origin,$width=false){
  ...
  $elementName = '<span '.$gotoAndStyle.'><div style="position:absolute;top:3px;left:43px;">'.formatIcon($objectClass, 22).'</div>&nbsp;'.i18n(str_replace('Simple','',$objectClass)).'&nbsp;#'.$objectId.'</span>';
  if ($origin=='activityStream') {
