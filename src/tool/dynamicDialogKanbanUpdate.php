...
if ($typeDynamic == 'update') {
	...
	if($extraRequiredFields){
		...
		foreach ($extraRequiredFields as $field){
	     $requiredField[trim($field)]=$obj->getDataType(trim($field));
	     $flatRequired[trim($field)]=trim($field);
		}
	}
	$flatRequired = trim(implode(',', $flatRequired));
	$dateWidth='72';
...
<?php
	...
	if(count($requiredField)>0){
      foreach ($requiredField as $id=>$type){
        if(substr($id, 0, 2) == 'id' and $type == 'int'){
          ?>
          <tr>
			<td>
				<div class="dialogLabel"><?php echo i18n("col".ucfirst($id));?></div>
			</td>
		  </tr>
		  <tr>
			<td>
          ...
          <tr>
			<td>
				<div class="dialogLabel"><?php echo i18n("col".ucfirst(trim($id)));?></div>
			</td>
		  </tr>
		  <tr>
			<td>
          ...
          if (sessionValueExists('browserLocaleDateFormatJs')) {
          	$min='';
          	if (substr($id, -7)=="EndDate") {
          		$start=str_replace("EndDate", "StartDate", $id);
          		if (property_exists($obj, $start)&&property_exists($obj, 'refType')&&$obj->refType!="Milestone") {
          			$min=$obj->$start;
          		} else {
          			$start=str_replace("EndDate", "EisDate", $id);
          ...
          echo $colScript;
          echo '</div>';
          $fmtDT='time'; // valTime=substr($valTime,0,5);
		  ...
		  <tr>
			...
			$isWork=false;
			$isCost=false;
			if(strpos($id, 'Work'))$isWork=true;
			if(strpos($id, 'Cost'))$isCost=true;
