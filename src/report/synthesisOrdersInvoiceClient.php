...
echo' <td class="reportTableHeader"  style="width:'.$col3.'%">'.ucfirst(i18n('colDate')).'</td>';
echo' <td class="reportTableHeader"  style="width:'.$col2.'%">'.ucfirst(i18n('Status')).'</td>';
echo' <td class="reportTableHeader"  style="width:'.$col3.'%">'.ucfirst(i18n('commandAmount')).'</td>';
if($paramshowReference){
  echo' <td class="reportTableHeader"  style="width:'.$col2.'%">'.ucfirst(i18n('colReference')).'</td>';
}
...
echo' <td class="reportTableHeader"  style="width:'.$col3.'%">'.ucfirst(i18n('colDate')).'</td>';
echo' <td class="reportTableHeader"  style="width:'.$col2.'%">'.ucfirst(i18n('Status')).'</td>';
echo' <td class="reportTableHeader"  style="width:'.$col3.'%">'.ucfirst(i18n('colBillAmount')).'</td>';
...
foreach ($tabCmdOrder as $wbs=>$objId){
  foreach ($objId as $obj){
    foreach ($obj as $val){
      ...
      echo' <td  rowspan="'.$nbRowSpan.'" class="reportTableData"  style="padding:4px;text-align:center;width:'.$col3.'%">'.htmlFormatDate($val->receptionDate).'</td>';
      ...
      echo' <td  rowspan="'.$nbRowSpan.'" class="reportTableData"  style="padding:4px;width:'.$col3.'%;text-align:right;">'.htmlDisplayCurrency($val->totalUntaxedAmount).'</td>';
      if(isset($tabBill[$val->id])){
        $i=0;
        foreach ($tabBill[$val->id] as $billId){
          ...
          echo' <td  rowspan="'.$nbRowSpan.'" class="reportTableData"  style="padding:4px;text-align:center;width:'.$col3.'%">'.htmlFormatDate($newBill->date).'</td>';
          echo' <td  rowspan="'.$nbRowSpan.'" class="reportTableData"  style="padding:4px;width:'.$col3.'%"><div>'.formatColor('Status', $newBill->idStatus).'</div></td>';
          echo' <td   class="reportTableData"  style="padding:4px;width:'.$col3.'%;text-align:right;">'.htmlDisplayCurrency($newBill->untaxedAmount).'</td>';
          $backGround = "";
