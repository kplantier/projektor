                        ...
                        <tr>
                            <td class="reportTableData">
                                ...
                                if (substr($atch->mimeType, 0, 5) == 'image') {
                                    ?>
                                    <img src="../tool/download.php?class=Attachment&amp;id=<?= $atch->id ?>&amp;nodl=1&csrfToken=<?php echo getSessionValue('Token');?>" alt="<?= $atch->fileName ?>" />
                                    <?php
                                } else {
                                    ?>
                                    <object data="../tool/download.php?class=Attachment&amp;id=<?= $atch->id ?>&amp;nodl=1&csrfToken=<?php echo getSessionValue('Token');?>" type="<?= $atch->mimeType ?>"<?= (($atch->mimeType == 'application/pdf') ? ' style="width:100%;min-height:400px;"' : '') ?>>
                                        <a href="../tool/download.php?class=Attachment&amp;id=<?= $atch->id ?>&csrfToken=<?php echo getSessionValue('Token');?>">Download <?= $atch->fileName ?></a>
                                    </object>
                                    <?php
                                }
                                ?>
