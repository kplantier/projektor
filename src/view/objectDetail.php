...
function drawHistoryFromObjects($refresh=false) {
  ...
  foreach ($historyList as $hist) {
    ...
    if (!$hide) {
      ...
      $oldValue=str_replace(array('<style','/style>'),array('<nostyle','/nostyle>'),$oldValue);
      $newValue=str_replace(array('<style','/style>'),array('<nostyle','/nostyle>'),$newValue);
  ...
  foreach ($attachments as $attachment) {
    ...
    if ($user->id==$attachment->idUser or $attachment->idPrivacy==1 or ($attachment->idPrivacy==2 and $ress->idTeam==$attachment->idTeam) ) {
      ...
      if ($attachment->isThumbable()) {
        ...
        echo '<div style="float:left;cursor:pointer" onClick="showLink(\''.htmlEncodeUrl($attachment->link).'\');">';
...
function drawWorkCommand($list,$obj){
  ...
  echo '  <td colspan=2 class="reportTableHeader" style="width:20%">' . ucfirst(i18n('ordered')) . '</td>';
  echo '  <td colspan=2 class="reportTableHeader" style="width:20%">' . ucfirst(i18n('used')) . '</td>';
...
function drawChecklistFromObject($obj,$nbCol=3) {
  ...
  if($moduleCheckList){
    $displayChecklist=(Parameter::getGlobalParameter('displayChecklist')!='')?Parameter::getGlobalParameter('displayChecklist'):'YES';
