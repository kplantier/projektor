  ...
  menuLeft.prototype = {
    ...
    _resizeDiv : function(globalWidth){
      ...
      if(!isObject){
        ...
        var listDivWidth=(globalWidth-detailWidth);
        var hideDetail=false;
        if(dojo.attr(dojo.byId("listDiv"),"region")=='top'){
          listDivHeight=mainDivHeight - detailHeight;
          listDivWidth=globalWidth-20;
          if(this.isMenuOpen!='true')detailWidth+=250;
          else detailWidth-=250;
          if(detailHeight==0)hideDetail=true;
        }else if(detailWidth!=0){
          if(this.isMenuOpen!='true'){
            listDivWidth-=250;
            if(detailWidth!=0) detailWidth+=250;
          }else{
            listDivWidth+=250;
            detailWidth-=250;
          }
        ...
        dojox.fx.combine([ 
          ...
          properties : {
            ...
            //left: { start:(this.isMenuOpen=='true')? 250 : 0 ,end:(this.isMenuOpen=='true')? 0 : 250}
          },
      ...
      if(!formChangeInProgress && dojo.byId('id') && dojo.byId('id').value && !detailHidden && !hideDetail) {
        var param=(isObject)?"":"?planningType="+currentScreen;
          setTimeout('loadContent("objectDetail.php'+param+'", "detailDiv", "listForm");',
              duration +5);
      }
    }
  };
