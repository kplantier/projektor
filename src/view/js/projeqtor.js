...
var tabPlanView=['Planning','GlobalPlanning','PortfolioPlanning','ResourcePlanning','VersionsPlanning','ContractGantt','HierarchicalBudget'];
...
// MTY - LEAVE SYSTEM    
      ...
      if ((grid || dojo.byId("GanttChartDIV")) && dojo.byId("detailFormDiv")
        ...
        if (lastOperation.value == "copy") {
          ...
          if(coverListAction=='CLOSE')notShowDetailAfterReplan=true;
          loadContent("objectDetail.php?refresh=true", "detailFormDiv", 'listForm');
...
function saveCollapsed(scope,callBack) {
  ...
  dojo.xhrPost({
    url : "../tool/saveCollapsed.php?scope=" + scope + "&value=true&csrfToken="+parent.csrfToken,
    handleAs : "text",
    load : function(data, args) {
...
function saveExpanded(scope,callBack) { 
  ...
  dojo.xhrPost({
    url : "../tool/saveCollapsed.php?scope=" + scope + "&value=false&csrfToken="+parent.csrfToken,
    handleAs : "text",
    load : function(data, args) {
      if (callBack) setTimeout(callBack, 10);
    }
...
function afterLoginCheckCallback() {
  ...
  if (dojo.byId('pluginCustomFixDefinition')) {
    if (afterLoginCheckCallbackLast==dojo.byId('pluginCustomFixDefinition').value) return; // Stop is same field fixed twice (not working fix)  
...
function getExtraHiddenFields(idType,idStatus,idProfile) {
  ...
  dojo.xhrGet({
    url : "../tool/getExtraHiddenFields.php" + "?type=" + idType+"&status="+idStatus+"&profile="+idProfile
        + "&objectClass=" + objectClass+"&csrfToken="+csrfToken,
    handleAs : "text",
    load : function(data) {
      var obj = JSON.parse(data);
...
function setDefaultPlanningMode(typeValue) {
  dojo.xhrGet({
    url : '../tool/getSingleData.php?dataType=defaultPlanningMode&idType='
        + typeValue + "&objectClass=" + dojo.byId('objectClass').value+'&csrfToken='+csrfToken,
    handleAs : "text",
    load : function(data) {
      var objClass = dojo.byId('objectClass').value;
...
function setDefaultCategory(typeValue) {
  dojo.xhrGet({
    url : '../tool/getSingleData.php?dataType=defaultCategory&idType='
        + typeValue + "&objectClass=" + dojo.byId('objectClass').value +'&csrfToken='+csrfToken+'&csrfToken='+csrfToken,
    handleAs : "text",
    load : function(data) {
...
function checkValidatedSize(paramDiv,paramRightDiv, paramMode, showDetailDiv){
  ...
  if(isNotObject && coverListAction=='CLOSE')return; //florent coverList
  if (paramMode != 'switch'){
    ...
    if (paramDiv== 'left'){
      if(hideEmptyDetail && dojo.byId("contentDetailDiv") && dojo.byId("noDataInObjectDetail")) {
	    ...
	    if( dojo.byId('detailRightDiv').offsetHeight==0  || paramRightDiv=='trailing' || dojo.byId("contentDetailDiv").offsetHeight<250){
        if(isNotObject && dojo.byId('detailDivHeightSize')){
	      ...
	      if (dojo.byId("contentDetailDiv").offsetHeight<250) {
	        ...
	        dijit.byId("contentDetailDiv").resize({h: minHeight});
	        resizeContainer("mainDivContainer", null);
	        return true;
	      }
	    }else{
	  ...
	  checkValidatedSizeRightDivRun=true;
		if(paramDiv== 'left'){
		   ...
		   if(coverListAction!='CLOSE' && dojo.byId('detailRightDiv').offsetHeight<15 && typeof showActivityStreamVar !== 'undefined'  && showActivityStreamVar==true) {
		     dijit.byId('detailRightDiv').resize({h:lastdetailheightActivityStream});
         resizeContainer("mainDivContainer", null);
         checkValidatedSizeRightDivRun=false;
         return true;
		    }
...
function refreshObjectDivAfterResize() {
  ...
  if( multiSelection==false){
    if (!formChangeInProgress && dijit.byId('id')) { 
      ...
      var param=(!isObject)?'&planningType='+currentScreen:'';
      setTimeout('if (dojo.byId("buttonDiv")) loadContent("objectButtons.php?refreshButtons=true'+param+'","buttonDiv", "listForm",false,false,false,false,'
...
var visibleBudgetList = new Object();
	...
	if(visible=='hidden'){// open
		...
		for(var i=0;i<subBudgetList.length;i++){
			if (dojo.byId('hierarchicalBudgetRow_'+subBudgetList[i]) ){
				dojo.byId('hierarchicalBudgetRow_'+subBudgetList[i]).style.display = 'table-row';
				dojo.setStyle('hierarchicalBudgetRow_'+subBudgetList[i], 'visibility', 'visible');
				if(Object.keys(visibleBudgetList).includes('id_'+subBudgetList[i])){
					if(getValueWithKeyOnObject ('id_'+subBudgetList[i],visibleBudgetList)=='hidden') break;
				}
			}
		}
	}else{// close
...
function saveGanttElementResize(element,refId,id,dateStart,dateEnd,duration){
  ...
  dojo.xhrGet({
    ...
    load : function(data, args) {
      ...
      if((dojo.byId('objectClass').value.trim() !='' && dojo.byId('objectId').value.trim() !='' && dojo.byId('automaticRunPlan') && dojo.getAttr('automaticRunPlan','aria-checked')!='true') && dojo.byId('objectId').value.trim()==refId.trim() ){
        loadContent("objectDetail.php", "detailDiv", 'listForm');
        loadContentStream();
      }
    },
...
function addSubTaskRow(id,refType,refId,sortOrder,resourceFilter,priorityFilter,nameTodoList){
                          ...
                          newButtonAttach=document.createElement('input'),
                            slidContainerDiv=newSubTask.querySelector('#'+refType+'_'+refId+'_slidContainer_0'),
                              divAttachment=newSubTask.querySelector('#'+refType+'_'+refId+'_divAttachement_0'),
                                newDivExtra=document.createElement('input'),
                                  newButtonCopy=document.createElement('input'),
                                    newButtonDeleteSubTask=document.createElement('input'),
                                      extraButtonsDetailDiv=newSubTask.querySelector('#'+refType+'_'+refId+'_extraButtonsDetailDiv_0');
  ...
  if(buttonAddAttachExist){
    ...
    dojo.connect(newButtonAttach,'onBegin' , function(value){saveAttachment(true,id+'_attachmentFile');});
  ...
  dojo.connect(newDivExtra,'onClick' , function(){showExtraButtons(id+'_extraButtonsDetail');}); 
  dojo.connect(newButtonCopy,'onClick' , function(){ copySubTaskObjectBox('SubTask',id);}); 
  dojo.connect(newDeleteSubTask,'onClick' , function(){ deleteSubTask(id,refType,refId,nameTodoList);});
...
function nextSlides(op,id,refType,refId) {
  var pos=dojo.byId(refType+'_'+refId+'_pos_'+id).value,
      n=1;
...
function showSlides(n,id,refType,refId) {
  ...
  var prevButton=dojo.byId(refType+'_'+refId+'_prev_'+id),
        nextButton=dojo.byId(refType+'_'+refId+'_next_'+id);
  status=n-1;
  switch(status){
    case '0':
