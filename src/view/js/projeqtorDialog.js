...
function ShowDetailScreen(){
  ...
  checkValidatedSize(paramDiv,paramRightDiv, paramMode,'true');
  if(dojo.byId('detailRightDiv').offsetHeight>10){
    showActivityStreamVar=true;
  }
  if(displayAnotherDetail){
    setTimeout('loadContent("objectDetail.php?planning=true&planningType="+dojo.byId("objectClassManual").value,"detailDiv","listForm",);',300);
    setTimeout('dojo.byId("detailDiv").style.display="block";',325);
  }
}
...
function showImage(objectClass, objectId, imageName) {
  if (objectClass == 'Affectable' || objectClass == 'Resource'
    ...
    imageUrl="../files/thumbs/Affectable_" + objectId + "/thumb80.png"+"&csrfToken="+csrfToken;
  }else if(objectClass == 'Note'){
	  imageUrl=objectId+"&csrfToken="+csrfToken;
  }else {
...
function showLink(link) {
  ...
  window.frames['showHtmlFrame'].location.href=link+"&csrfToken="+csrfToken;
