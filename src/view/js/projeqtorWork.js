...
function submitWorkPeriod(action) {
  ...
  dojo.xhrGet({
  url: '../tool/submitWorkPeriod.php?action='+action+'&rangeType='+rangeType+'&rangeValue='+rangeValue+'&resource='+resource+'&csrfToken='+csrfToken,
    handleAs: "text",
...
function sendAlertOnSubmitWork(action, rangeType, rangeValue, resource){
	dojo.xhrGet({
		  url: '../tool/sendMail.php?className=Imputation&action='+action+'&rangeType='+rangeType+'&rangeValue='+rangeValue+'&resource='+resource+'&csrfToken='+csrfToken,
		    handleAs: "text",
...
function validFutureWorkDate(){
  valid=function() {
    ...
    submitForm("../tool/saveImputation.php?csrfToken="+csrfToken,"resultDivMain", "listForm", true);
  };
